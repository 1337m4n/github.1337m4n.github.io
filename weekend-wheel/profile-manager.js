...
async function readSnapshot(){
  ...
  var r=await fetch(api()+"?ref="+encodeURIComponent(BRANCH)+"&t="+Date.now(),{headers:headers(token),cache:"no-store"});
  ...
  return {token:token,sha:meta.sha,doc:normalize(doc)};
}

async function writeSnapshot(snapshot,mutator,message){
  if(busy)throw new Error("BUSY");
  busy=true;
  setPanelBusy(true);
  try{
    var doc=normalize(snapshot.doc);
    mutator(doc);
    doc.version=2;
    doc.updatedAt=now();
    ...
    if(!p.ok){
      ...
      if(p.status===409||p.status===422)throw new Error("CONFLICT");
...
@media(max-width:760px){.profileGrid{grid-template-columns:1fr}.profilePanel{padding:12px!important}.profileBtns button{min-height:42px}}
`;
...
function buildPanel(){
  ...
  p.id="profilePanel";
  p.className="panel adminOnlyBlock profilePanel";
  ...
  if(editor&&editor.parentNode)editor.parentNode.insertBefore(p,editor);
  else if(document.querySelector(".wrap"))document.querySelector(".wrap").appendChild(p);
...
function render(){
  ...
  for(var i=0;i<3;i++)(function(idx){
    ...
    var active=state.activeSlot===idx+1;
    var card=document.createElement("div");
    card.className="profileCard"+(active?" active":"")+(!slot?" profileEmpty":"");
    ...
    var input=card.querySelector(".profileName");
    var sw=card.querySelector(".profileSwitch");
    var save=card.querySelector(".profileSave");
    input.value=slot?slot.name:("配置 "+(idx+1));
    sw.disabled=!slot||active;
    sw.addEventListener("click",function(){switchTo(idx+1);});
    save.addEventListener("click",function(){saveTo(idx+1,input.value);});
...
function hasUnsaved(doc){
  var cur=doc&&doc.slots&&doc.slots[doc.activeSlot-1];
  return !!(cur&&!sameItems(wheelItems(),cur.items));
}
...
async function saveActive(){
  ...
  try{
    ...
    if(next){
      if(window.WeekendWheelApp&&window.WeekendWheelApp.saveLocal)window.WeekendWheelApp.saveLocal();
      alert("当前配置已保存到槽位 "+slotNo+"。");
    }
  }catch(e){handleError(e);}
}
...
async function saveTo(slotNo,name){
  ...
  try{
    ...
    state=snap.doc;
    render();
    var occupied=snap.doc.slots[slotNo-1];
...
async function newProfile(){
  ...
  try{
    ...
    render();
    if(!confirmLoseUnsaved(snap.doc))return;
    var empty=snap.doc.slots.findIndex(function(s){return!s});
    var def=empty>=0?empty+1:1;
    var raw=prompt("新建到哪个槽位？请输入 1、2 或 3。",String(def));
    if(raw===null)return;
    var slotNo=parseInt(raw,10);
    if(!(slotNo>=1&&slotNo<=3)){alert("请输入 1、2 或 3。");return;}
    if(snap.doc.slots[slotNo-1]){
...
function handleError(e){
  ...
  else if(m==="CONFLICT")alert("线上配置在你操作期间被其他设备更新了。为避免覆盖，本次操作已取消，请刷新配置库后重试。");
  else if(m==="BUSY")alert("配置库正在处理上一项操作，请稍后再试。");
